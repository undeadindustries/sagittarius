// Package diff renders git-style unified diffs between two strings. It is a
// leaf package (stdlib only) so it can be shared by internal/snapshot (for
// /diff and /undo) and internal/tools (for write_file confirmation previews
// and results) without coupling those packages to each other.
package diff
